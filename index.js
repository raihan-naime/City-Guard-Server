  ...
  try {
    ...
    const verifyStaff = async (req, res, next) => {
      ...
      if (user?.role !== "staff" && user?.role !== "admin") {
        return res
          .status(403)
          .json({ message: "Forbidden access: Staff or Admin only" });
      }
      next();
    };
      ...
      try {
        ...
        if (user.subscriptionStatus === "free") {
          ...
          if (issueCount >= 3) {
            return res
              .status(403)
              .json({ message: "Free limit reached. Upgrade to Premium." });
          }
        }
        ...
        const insertedIssue = await issuesCollection.findOne({
          _id: result.insertedId,
        });
        ...
        const pipeline = [
          ...
          // Project fields (optional, to clean up)
          {
            $addFields: {
              assignedTo: {
                $cond: {
                  if: { $gt: ["$assignedToDetails._id", null] },
                  then: {
    ...
    app.patch("/issues/:id/upvote", verifyToken, async (req, res) => {
      try {
        ...
        if (issue.author.toString() === user._id.toString()) {
          return res
            .status(400)
            .json({ message: "Cannot upvote your own issue" });
        }
    ...
    app.patch(
      ...
      async (req, res) => {
        try {
          ...
          console.log(
            `Assigning staff ${staffId} to issue ${req.params.id} by ${req.user.email}`
          );
          ...
          if (staff.role !== "staff") {
            ...
            return res
              .status(400)
              .json({ message: "User is not a staff member" });
          }
    ...
    app.put("/issues/:id", verifyToken, async (req, res) => {
      try {
        ...
        if (issue.status !== "pending") {
          return res
            .status(400)
            .json({ message: "Cannot edit non-pending issues" });
        }
    ...
    app.post("/payments", verifyToken, async (req, res) => {
      try {
        ...
        res
          .status(200)
          .json({ success: true, payment, message: "Payment successful" });
      } catch (error) {
        res.status(500).json({ message: error.message });
      }
    });
    ...
    app.get("/stats/staff", verifyToken, verifyStaff, async (req, res) => {
      try {
        ...
        const todayTasks = assignedIssues.filter(
          (i) => i.status === "pending" || i.status === "in-progress"
        ).length;

        const statusDistribution = {
          pending: assignedIssues.filter((i) => i.status === "pending").length,
          inProgress: assignedIssues.filter((i) => i.status === "in-progress")
            .length,
          resolved: assignedIssues.filter((i) => i.status === "resolved")
            .length,
